...
- **Resume page preview now fits the viewer card properly** — the custom PDF page carousel could render overscaled/cropped pages after the earlier viewer rewrite. `ResumeCarousel` now measures the available track width, renders each PDF page at a fitted width, and displays one full page per horizontal slide so resumes are readable without the page being zoomed far past the card bounds
