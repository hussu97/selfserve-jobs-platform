...
### Added
- **Cleanup job deploy workflow** — added a manual GitHub Actions workflow that validates required GCP/internal secrets and creates or updates the `cleanup-stale-data` Cloud Scheduler job against the deployed `selfserve-jobs-customer-api` service.
