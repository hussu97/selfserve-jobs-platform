...
- **Resume page preview now fits the viewer card properly** — replaced the lower-level custom PDF rasterization path with a `react-pdf` based viewer and kept the custom Sage & Stone swipe UI around it. Resume pages now size to the available card width and render as full horizontal slides instead of appearing oversized or cropped
