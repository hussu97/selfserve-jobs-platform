...
### Fixed
- **Cloud SQL connection exhaustion on job listing reads** — reduced the API SQLAlchemy pool defaults from 30 possible connections per Cloud Run instance to 2, added env-tunable pool size/overflow/timeout/recycle settings, and documented the runtime knobs so `/api/v1/jobs` traffic does not exhaust PostgreSQL connection slots under scale-out.
