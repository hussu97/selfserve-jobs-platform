    ...
    <article className="max-w-4xl mx-auto">
      ...
      <header className="mb-8">
        <div className="flex flex-wrap items-start gap-3 mb-3">
          ...
          {job.deadline_date && (
            <span className="text-sm px-2.5 py-1 rounded-full border font-medium text-primary border-primary bg-primary/5">
              Deadline: {formatDeadline(job.deadline_date)}
            </span>
          )}
        </div>

        <h1 className="text-3xl sm:text-4xl font-bold mb-2 text-secondary">
          {job.job_title}
        </h1>
          ...
          <div className="rounded-xl border border-border bg-surface p-5">
            ...
            {job.contact_method === 'email' && job.contact_email ? (
              <a
                ...
                className="inline-flex items-center justify-center w-full gap-2 px-4 py-3 rounded-xl text-white font-medium transition-opacity hover:opacity-90 bg-primary"
              >
                <svg className="h-4 w-4" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
