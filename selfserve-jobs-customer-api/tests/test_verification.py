...
from app.services.code_generator import generate_token, generate_verification_code
...
async def test_verify_expired_code_rejected(client, db_session):
    ...
    verification = await _make_verification(
        db_session, entity_type="job", entity_code=job.job_code, email=job.email, expired=True
    )
...
async def test_verify_job_activates_all_pending_for_email(client, db_session):
    """Verifying one code activates ALL pending listings for that email."""
    ...
    verification = await _make_verification(db_session, entity_type="job", entity_code=job1.job_code, email=email)
...
async def test_verify_profile_code_activates_profile(client, db_session):
    ...
    verification = await _make_verification(
        db_session, entity_type="profile", entity_code=profile.profile_code, email=email
    )
...
async def test_verify_code_cannot_be_used_twice(client, db_session):
    ...
    verification = await _make_verification(db_session, entity_type="job", entity_code=job.job_code, email=email)
...
async def test_resend_job_success(client, db_session):
    job = await _make_active_job(db_session, job_code="resendj0001", email="resend@example.com")
...
async def test_resend_rate_limit_exceeded(client, db_session):
    ...
    job = await _make_active_job(db_session, job_code="ratelimitj1", email="ratelimit@example.com")

    with patch("app.services.email_service.send_verification_email", new_callable=AsyncMock):
        for _ in range(RESEND_LIMIT_PER_ENTITY):
            r = await client.post(
                "/api/v1/verify/resend",
                json={"entity_type": "job", "email": job.email, "entity_code": job.job_code},
            )
