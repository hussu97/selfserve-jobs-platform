...
def build(login_url: str) -> tuple[str, str, str]:
    """Return (subject, html_body, text_body) for an admin magic login email."""
    subject = "Admin sign-in link — hirebridge"
    ...
    body_html = f"""
      ...
      <p style="margin:0 0 28px 0;font-family:'Helvetica Neue',Arial,sans-serif;
        ...
        Click the button below to sign in to the admin portal. This link expires in&nbsp;
        <strong>15&nbsp;minutes</strong> and can only be used once.
      </p>
      <div style="margin:0 0 28px 0;">
        {cta_button(login_url, "Sign in to Admin Portal")}
      </div>
      {fallback_link(login_url)}
    """

    html_body = shell(
        header_text="Admin sign-in",
        body_html=body_html,
        footer_note="If you did not request this, you can safely ignore this email.",
    )

    text_body = f"""Admin sign-in link — hirebridge

Click the link below to sign in to the admin portal (expires in 15 minutes, single use):
{login_url}
